...
import com.cwj.yudada.model.dto.app.*;
...
import com.cwj.yudada.model.vo.AppVO;
...
public class AppController {
    ...
    public BaseResponse<Boolean> doAppReview(@RequestBody ReviewRequest reviewRequest, HttpServletRequest request){
        ...
        ReviewStatusEnum reviewStatusEnum = ReviewStatusEnum.getEnumByValue(reviewRequest.getReviewStatus());
        if (reviewStatusEnum == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR);
        }
        App oldApp = appService.getById(reviewRequest.getId());
        if (oldApp == null){
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR,"该应用不存在");
        }
        if (oldApp.getReviewStatus().equals(reviewRequest.getReviewStatus())){
            throw new BusinessException(ErrorCode.OPERATION_ERROR,"该审核状态已更新");
        }
        User user = userService.getLoginUser(request);

        oldApp.setId(reviewRequest.getId());
        oldApp.setReviewStatus(reviewRequest.getReviewStatus());
        oldApp.setReviewerId(user.getId());
